...
"""Testing suite for the PyTorch OLMoE2 model."""

import unittest

from parameterized import parameterized

from transformers import Olmoe2Config, is_torch_available, set_seed
...
class Olmoe2ModelTester:
    ...
    def create_and_check_model(
        ...
        model = Olmoe2Model(config=config)
    ...
    def create_and_check_for_causal_lm(
        ...
        model = Olmoe2ForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class Olmoe2ModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (Olmoe2Model, Olmoe2ForCausalLM) if is_torch_available() else ()
    test_pruning = False
    fx_compatible = False
    ...
    def setUp(self):
        self.model_tester = Olmoe2ModelTester(self)
        self.config_tester = ConfigTester(self, config_class=Olmoe2Config, hidden_size=37)
    ...
    @unittest.skip(reason="OLMoE2 does not support head pruning.")
    ...
    @unittest.skip(reason="OLMoE2 buffers include complex numbers, which breaks this test")
    ...
    def test_model_rope_scaling(self, scaling_type):
        ...
        original_model = Olmoe2Model(config)
        ...
        scaled_model = Olmoe2Model(config)
...
@require_torch
class Olmoe2IntegrationTest(unittest.TestCase):
    @slow
    def test_model_7b_logits(self):
        input_ids = [[1, 306, 4658, 278, 6593, 310, 2834, 338]]
        model = Olmoe2ForCausalLM.from_pretrained("allenai/OLMoE2-1B-7B-0924", device_map="auto")
    ...
    def test_model_7b_greedy_generation(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained("allenai/OLMoE2-1B-7B-0924", device_map="auto")
    ...
    def test_fast_special_tokens(self):
        fast_tokenizer = GPT2TokenizerFast.from_pretrained("allenai/OLMoE2-1B-7B-0924")
    ...
    def test_simple_encode_decode(self):
        rust_tokenizer = GPT2TokenizerFast.from_pretrained("allenai/OLMoE2-1B-7B-0924")
